...
DeviceGetAppPreferences(void *prefs, SHORT *prefSize, BOOLEAN saved)
{
  ...
  if (DataFindDB((BYTE*)("Gmbt_preferences"),&dbID)) {
...
DeviceSetAppPreferences(void *prefs, SHORT prefSize, BOOLEAN saved)
{
  ...
  if (!DataFindDB((BYTE*)("Gmbt_preferences"),&dbID)) {
    DataNewDB((BYTE*)("Gmbt_preferences"),1,(BYTE*)("VTech Information"),&dbID);
  }
  DataOpenDB(dbID,0,OPEN_RW);
...
DeviceKeyCurrentState()
{
...

  {
    ...
    if (pinKeys & 0x02) result       |= keyBitCustom3;
    if (pinKeys & 0x04) result       |= keyBitCustom4;
